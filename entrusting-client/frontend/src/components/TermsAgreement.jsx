...
const TermsAgreement = ({ onComplete }) => {
  ...
  const agreementList = [
    { id: 'age', title: '만 14세 이상이며, 본인 명의 계좌임을 확인합니다', required: true, desc: "금융거래를 위한 기본 연령 확인입니다." },
    { id: 'terms', title: 'Continue Bank 서비스 이용약관', required: true, hasDetail: true, desc: "거래 한도, 분쟁 처리 등 서비스 전반에 대한 약속입니다." },
    { id: 'privacy', title: '개인정보 수집 및 이용 동의', required: true, hasDetail: true, desc: "이름, 연락처 등 서비스 제공에 필요한 정보를 수집합니다." },
    { id: 'uniqueId', title: '고유식별정보(주민번호 등) 처리 동의', required: true, hasDetail: true, desc: "금융거래 및 본인확인을 위해 민감정보를 처리합니다." },
    { id: 'creditInfo', title: '신용정보 조회 및 제공 동의', required: true, hasDetail: true, desc: "신용 평가 및 감독 규정 준수를 위해 신용 정보를 조회합니다." },
    { id: 'electronicFinance', title: '전자금융거래 기본약관 동의', required: true, hasDetail: true, desc: "비밀번호 관리, 손실배상 등 전자금융 기본 수칙입니다." },
    { id: 'monitoring', title: '금융거래 정보 모니터링 및 기록 동의', required: true, hasDetail: true, desc: "자금세탁방지(AML) 및 모니터링을 위한 법적 절차입니다." },
    { id: 'marketingPersonal', title: '개인맞춤형 금융상품 추천 동의 (선택)', required: false, hasDetail: true, desc: "고객님의 패턴을 분석해 꼭 필요한 상품을 추천해 드려요." },
    { id: 'marketing', title: '혜택 및 이벤트 소식 받기 (문자 메시지) (선택)', required: false, hasDetail: true, desc: "특별 혜택과 이벤트 소식을 가장 먼저 보내드릴게요." }
  ];
    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <main className="flex-1 px-8 py-10 max-w-[520px] mx-auto w-full overflow-y-auto scrollbar-hide">
        ...
        <div className="space-y-6">
          ...
          <div className={`flex items-center gap-2 transition-all duration-500 ${visibleItems >= 3 ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <span className="text-sm font-bold text-gray-500">필수 동의 항목 (모두 동의해야 가입 가능)</span>
          </div>
          ...
          <div className="space-y-1">
            {agreementList.map((item, index) => (
              <div key={item.id} className={`transition-all duration-500 transform ${visibleItems >= index + 4 ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                {index === 7 && ( // Index 7 is the first optional item (marketingPersonal)
                   <div className="flex items-center gap-2 mt-10 mb-4 opacity-100">
                    <span className="text-sm font-bold text-gray-500">선택 동의 항목 (동의하지 않아도 가입 가능)</span>
                  </div>
                )}
                <AgreementItem 
        ...
        <div className={`mt-10 flex items-center justify-center gap-2 transition-all duration-1000 ${visibleItems >= 12 ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <p className="text-[12px] text-gray-300 font-medium">개인정보는 AES-256 방식으로 안전하게 보관됩니다.</p>
        </div>
      </main>

      {/* Action Button (금융사 스타일의 단일 버튼) */}
      <div className={`px-8 pb-12 max-w-[520px] mx-auto w-full transition-all duration-700 ${visibleItems >= 12 ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
        <button 
          ...
          `}
        >
          {allChecked ? '동의하고 계속하기' : (allRequiredChecked ? '동의하고 계속하기' : '필수 항목에 동의해 주세요')}
        </button>
      </div>
    </div>
  );
...
const AgreementItem = ({ checked, onChange, required, title, desc, hasDetail, isExpanded, onExpand, isVisible }) => {
  ...
  return (
    <div className="border-b border-gray-50 last:border-0 overflow-hidden">
      <div 
        className="group flex items-center py-4.5 px-2 cursor-pointer active:opacity-70 transition-all"
        onClick={onChange}
      >
        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-4 transition-all duration-300 ${checked ? 'border-transparent' : 'border-gray-200'} ${checkBg}`}>
          {checked && <Check size={14} className="text-white" strokeWidth={4} />}
        </div>
        
        <div className="flex-1 flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-[4px] ${labelBg} ${textColor} shrink-0`}>{required ? '필수' : '선택'}</span>
            <span className={`text-[15.5px] font-bold tracking-tight transition-colors ${checked ? 'text-[#1A1A1A]' : 'text-gray-500'}`}>{title}</span>
          </div>
        </div>

        {hasDetail && (
          <button onClick={onExpand} className="p-2 -mr-2 text-gray-300 hover:text-gray-500 transition-colors">
            {isExpanded ? <ChevronDown size={18} /> : <ChevronRight size={18} />}
          </button>
        )}
      </div>

      <div className={`px-2 pb-4 transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
